...
class SavGolFilt(object):
    ...
    def transform(self, myarray, y=None):
        ...
        G = scipy.dot(vande, scipy.dot(scipy.linalg.inv(R[0:vande.shape[1]]),
                                       scipy.transpose(scipy.linalg.inv(
                                           R[0:vande.shape[1]]))))  # Find the matrix of differentiators
        ...
        while last < array_size[0] - ((self.frame - 1) / 2):
            mid_array[last, :] = sum(
                (extract_B * myarray[last - ((self.frame - 1) / 2):last + ((self.frame - 1) / 2) + 1, :]),
                0)  # middle bit
            last = last + 1
        ...
        mid_array[0:(self.frame - 1) / 2, :], mid_array[array_size[0] - ((self.frame - 1) / 2):array_size[0],
